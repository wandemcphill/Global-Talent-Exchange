...
- Build-a-Son projection contract proof is now present too: `frontend/test/build_a_son/build_a_son_projection_contract_test.dart` blocks semantically invalid but parseable preview payloads before confirmation. Keep that with the Thread 3 product-flow proof batch.
- Competition settlement realtime authority is also verified: `backend/tests/realtime/test_competition_settlement_realtime.py` and `frontend/test/competitions/competition_settlement_state_test.dart` keep settlement events and lifecycle parsing backend-authoritative without inventing payout data. Keep that with the realtime/competitions proof batch.
- Competition bracket lifecycle aliases now map cleanly too: `frontend/test/compete/competition_bracket_models_test.dart` keeps `settlement_ready` and `awaiting_settlement` on the completed path without synthesizing extra bracket data. Keep that with the compete model batch.
- Withdrawal fee labels now derive from backend truth: `frontend/test/wallet/wallet_overview_canonical_state_test.dart` keeps quote/history labels driven by fee bps and amounts, not fixed copy. Keep that with the money-path UI proof batch.
- Creator/community truth framing is now tighter: `frontend/lib/features/creator/presentation/creator_canonical_surface.dart`, `frontend/lib/features/community/presentation/community_canonical_surface.dart`, and their tests now keep loading, empty, blocked, degraded, and backend-backed states distinct. Keep those with the creator/community scope-lock batch.
