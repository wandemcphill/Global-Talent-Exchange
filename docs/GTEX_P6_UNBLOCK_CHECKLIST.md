...
- the accepted 15-minute soak lane for the current phase passes end to end
...
| P6-05 | Add a committed 15-minute staging soak run | `QA` | `DO`, `UE`, `BE` | Define and run a 15-minute soak using the current-engine live playback path. Measure stability across the whole run, not only startup. Repo-side soak tooling is now committed via `tools/run_gtex_staging_soak.ps1`, with the execution lane documented in `docs/GTEX_P6_05_STAGING_SOAK_RUN.md`. For the current phase gate, the technical lead accepted the production-path 15-minute soak as the authoritative deployed-lane equivalent because no healthy separate staging API host was available during the gate window, and the later staging candidate `https://gtex-api-69rq.onrender.com` remained unhealthy during probe retries. | Soak report with date, environment, match ids, logs, screenshots, and summary metrics. CI or scripted invocation for the soak where practical. | The accepted deployed verification lane completes a continuous 15-minute run with stable live playback and no unresolved high-severity defects. | No accepted 15-minute run exists, the run aborts early, or critical defects remain unresolved. | `PASSED` |
...
- rollback runbook
- accepted 15-minute soak report for the current deployed verification lane
