...
| Money path | PASS focused, E2E pending | Transfer reservation suite passed in verifier handoff, wallet/treasury withdrawal fee focused tests passed, and wallet overview now reflects admin-selected gateway/manual rail truth with 111/111 targeted backend money-path tests green on 2026-06-09. Full money-path launch proof and frontend main-worktree gates remain pending. |
...
| Wallet overview rail truth | PASS targeted | `python -B -m pytest -p no:cacheprovider -q backend\tests\wallets\test_wallet_http.py -k "wallet_overview_surfaces_provider_status_and_live_restrictions or wallet_overview_marks_missing_korapay_secret_unavailable_in_production"`: 2 passed in 29.84s; broader money-path shard `backend\tests\treasury backend\tests\admin_finance backend\tests\wallets backend\tests\integration\test_payment_gateway.py` passed 111/111 in 510.85s. | Wallet overview now honors admin withdrawal `processor_mode` instead of pinning manual-bank-transfer truth, which keeps Korapay status aligned with production gateway availability. |
...
| Direct feature router catalog | PASS targeted | `frontend/test/active_shell_route_mount_test.dart` now proves `/player-cards`, `/player-cards/inventory`, `/competitions/gtex`, `/competitions/create`, `/competitions/streamer`, `/creator-share-market/clubs/:clubId`, and `/admin/creator-share-market/control` mount canonical feature screens through the live router instead of route unavailable. | Safe to stage with the same router/harness batch; this is the direct-route side of the canonical feature catalog. |
| Creator-share route wiring | PASS targeted | `frontend/test/gte_feature_routing_test.dart` now passes 20/20 and confirms creator-share club/admin routes mount the canonical screens with the expected club context. | Safe to stage with the Flutter route/product-flow batch once the surrounding dirty frontend files are reviewed together. |
...
| Flutter harness and route-test support | `frontend/pubspec.yaml`, `frontend/pubspec.lock`, `frontend/lib/router/app_router.dart`, `frontend/lib/features/shared/data/gte_json_support.dart`, `frontend/lib/features/shared/data/gte_feature_support.dart`, `frontend/lib/features/transfer_center/transfer_center_models.dart`, `frontend/lib/features/transfer_center/live_transfer_center_provider.dart`, `frontend/lib/features/market/presentation/widgets/market_models.dart`, `frontend/test/market/market_invariants_test.dart`, `frontend/test/active_shell_route_mount_test.dart` | Yes | Stage as one frontend test-harness batch after market invariant, active shell route, and route coverage commands remain green. `frontend/pubspec.*` belongs here because it adds the direct `test` dev dependency used by both test fixes. The router edit now also owns the canonical streamer engine aliases. |
| Regen discovery route wiring | `frontend/lib/features/regens/regens_screen.dart`, `frontend/test/regens/regens_screen_test.dart` | Yes | Stage as a small UI-route batch. The regens hero now opens Regen World directly from the lineage map action. |
