...
| Native 3D | placeholder | REMOVE | `app_destinations.dart`, `app_router.dart`, `features/match/match_screen.dart`, `features/match/match_native_3d_blocked_screen.dart` | The Matches tab still advertises a coming-soon disclosure, but the dedicated native-only route does not open a real live match contract yet. Remove it from active discovery until the route is real. |
