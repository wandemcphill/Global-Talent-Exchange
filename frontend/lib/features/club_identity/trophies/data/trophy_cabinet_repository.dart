...
class UnavailableTrophyCabinetRepository implements TrophyCabinetRepository {
  const UnavailableTrophyCabinetRepository();

  @override
  Future<TrophyCabinetDto> fetchTrophyCabinet({
    required String clubId,
    String? teamScope,
  }) => Future<TrophyCabinetDto>.error(
    const TrophyCabinetRepositoryException(
      'Live trophy cabinet repository is required.',
    ),
  );
  ...
  Future<HonorsTimelineDto> fetchHonorsTimeline({
    required String clubId,
    String? teamScope,
  }) => Future<HonorsTimelineDto>.error(
    const TrophyCabinetRepositoryException(
      'Live honors timeline repository is required.',
    ),
  );
  ...
  Future<SeasonHonorsArchiveDto> fetchSeasonHonors({
    required String clubId,
    String? teamScope,
  }) => Future<SeasonHonorsArchiveDto>.error(
    const TrophyCabinetRepositoryException(
      'Live season honors repository is required.',
    ),
  );

  @override
  Future<TrophyLeaderboardDto> fetchTrophyLeaderboard({String? teamScope}) =>
      Future<TrophyLeaderboardDto>.error(
        const TrophyCabinetRepositoryException(
          'Live trophy leaderboard repository is required.',
        ),
      );
}

enum TrophyRepositoryScenario { demo, empty, error }
...
class StubTrophyCabinetRepository implements TrophyCabinetRepository {
  ...
  Future<TrophyCabinetDto> fetchTrophyCabinet({
    ...
    await _delay();
    _throwIfNeeded('Trophy cabinet service is temporarily unavailable.');
    final List<Map<String, dynamic>> honors = _filteredHonors(
      clubId: clubId,
      teamScope: teamScope,
    );
    return TrophyCabinetDto.fromJson(
      _buildCabinetPayload(clubId: clubId, honors: honors),
    );
  }
  ...
  Future<HonorsTimelineDto> fetchHonorsTimeline({
    ...
    await _delay();
    _throwIfNeeded('Honors timeline could not be loaded.');
    return HonorsTimelineDto.fromJson(<String, dynamic>{
      'club_id': clubId,
  ...
  Future<SeasonHonorsArchiveDto> fetchSeasonHonors({
    ...
    await _delay();
    _throwIfNeeded('Season honors archive could not be loaded.');
    final List<Map<String, dynamic>> honors = _filteredHonors(
      clubId: clubId,
      teamScope: teamScope,
    );
    ...
    final List<Map<String, dynamic>> records = bySeasonAndScope.entries
        ...
        return <String, dynamic>{
          ...
          'elite_honors_count':
              seasonHonors
                  .where(
                    (Map<String, dynamic> item) =>
                        item['is_elite_honor'] == true,
                  )
                  .length,
          'recorded_at':
              DateTime.parse(
                first['earned_at'] as String,
      ...
      final int seasonComparison = (right['season_label'] as String).compareTo(
        left['season_label'] as String,
      );
      if (seasonComparison != 0) {
        return seasonComparison;
      }
      return (left['team_scope'] as String).compareTo(
        right['team_scope'] as String,
      );
    });

    return SeasonHonorsArchiveDto.fromJson(<String, dynamic>{
      'club_id': clubId,
      'club_name': _clubNames[clubId] ?? 'Expansion XI',
  ...
  Future<TrophyLeaderboardDto> fetchTrophyLeaderboard({
    ...
    await _delay();
    _throwIfNeeded('Trophy leaderboard could not be loaded.');
    if (scenario == TrophyRepositoryScenario.empty) {
      return TrophyLeaderboardDto.fromJson(<String, dynamic>{
        'entries': <dynamic>[],
      });
    }

    final List<Map<String, dynamic>> entries = _clubNames.entries
        .map((MapEntry<String, String> clubEntry) {
          final List<Map<String, dynamic>> honors = _filteredHonors(
            clubId: clubEntry.key,
            teamScope: teamScope,
          );
          final List<Map<String, dynamic>> categories = _buildCategoryPayload(
            honors,
          );
          ...
          return <String, dynamic>{
            ...
            'academy_honors_count':
                honors
                    .where(
                      (Map<String, dynamic> item) =>
                          item['team_scope'] == 'academy',
                    )
                    .length,
            'latest_honor_at': latest?.toUtc().toIso8601String(),
            'summary_outputs': categories
                .take(3)
                .map((Map<String, dynamic> item) {
                  return '${item['count']}x ${item['display_name']}';
                })
                .toList(growable: false),
            'continental_titles_count':
                honors.where((Map<String, dynamic> item) {
  ...
  List<Map<String, dynamic>> _filteredHonors({
    ...
    final Iterable<Map<String, dynamic>> filtered =
        teamScope == null
            ? honors
            : honors.where(
  ...
  List<Map<String, dynamic>> _buildCategoryPayload(
    ...
    final List<Map<String, dynamic>> categories = grouped.values
      ...
      ..sort((Map<String, dynamic> left, Map<String, dynamic> right) {
        ...
        return (left['display_name'] as String).compareTo(
  ...
  List<Map<String, dynamic>> _buildSeasonPayload(
    ...
    final List<Map<String, dynamic>> seasons = grouped.entries
        ...
        .toList(growable: false)
      ..sort((Map<String, dynamic> left, Map<String, dynamic> right) {
        return (right['season_label'] as String).compareTo(
          left['season_label'] as String,
        );
      });
    return seasons;
