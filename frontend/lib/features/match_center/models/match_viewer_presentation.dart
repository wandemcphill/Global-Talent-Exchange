...
enum MatchViewerPresentationMode { replay, broadcast }

enum BroadcastCameraPreset { broadcast, attackZoom, goalZoom, replayCamera }
...
class MatchPitchPresentation {
  ...
  MatchViewerPoint resolvePlayerPosition(MatchViewerPlayerFrame player) {
    ...
    final MatchViewerPlayerFrame? leftPlayer = _playerForFrame(
      leftFrame!,
      player.playerId,
    );
    final MatchViewerPlayerFrame? rightPlayer = _playerForFrame(
      rightFrame!,
      player.playerId,
    );
    ...
    return MatchViewerPoint(
      x:
          (player.position.x + (perpendicularX * curve))
              .clamp(0, 100)
              .toDouble(),
      y:
          (player.position.y + (perpendicularY * curve))
              .clamp(0, 100)
              .toDouble(),
    );
  }
