...
import 'package:gte_frontend/features/compete/domain/competition_rule_models.dart';
...
class CompetitionApi {
  ...
  Future<CompetitionBracketPayload?> fetchBracket(String competitionId) {
    return _withFallback<CompetitionBracketPayload?>(() async {
      final Object? payload = await _sendBest('GET', <String>[
        '/api/competitions/$competitionId/rounds',
      ]);
      return CompetitionBracketPayload.fromJson(
        _normalizeBracketPayload(competitionId, payload),
      );
    }, () async => null);
  }
...
Object? _normalizeBracketPayload(String competitionId, Object? payload) {
  if (payload is List) {
    return <String, Object?>{
      'competition_id': competitionId,
      'bracket': <String, Object?>{'rounds': payload},
    };
  }
  return payload;
}
...
class _CompetitionFixtureStore {
  ...
  String _nextInviteCode() {
    return 'INV-${(_idSequence++).toString().padLeft(6, '0')}';
  }
}
