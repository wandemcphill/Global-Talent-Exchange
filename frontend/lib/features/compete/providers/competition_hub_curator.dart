...
List<CompetitionSummary> competitionHubFeaturedCompetitions(
  ...
  final List<CompetitionSummary> sorted = competitions.toList(growable: true)
    ..sort(_compareSpotlight);
...
int _compareSpotlight(CompetitionSummary left, CompetitionSummary right) {
  final int statusCompare = _statusRank(
    left.status,
...
int _compareAcademyRows(CompetitionSummary left, CompetitionSummary right) {
  final int beginnerCompare = (right.beginnerFriendly == true ? 1 : 0)
      .compareTo(left.beginnerFriendly == true ? 1 : 0);
