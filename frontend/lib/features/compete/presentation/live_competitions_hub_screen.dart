...
import 'package:gte_frontend/shared/widgets/app_page_layout.dart';
    ...
    return AppPageLayout(
      ...
      subtitle:
          family == null
              ...
              : 'Backend brackets, standings, entries, and prize pools stay in dedicated arena routes.',
      ...
      children: <Widget>[
        GtexHeroPanel(
          ...
          description:
              family == null
                  ? 'Create competitions, join open entries, track backend standings, and review prize pools from one arena desk.'
                  : 'Join, manage, view backend rounds, check standings, and review results when your session allows it.',
          metrics: <Widget>[
            GtexStatTile(
              label: 'Official',
              value:
  ...
  Widget build(BuildContext context) {
      ...
      CompetitionFamilyRoute.hosted => hub.hostedCompetitions
          .map(
            (HostedCompetition item) => _competitionCard(
              ...
              title: item.title,
              subtitle:
                  '${item.status} | ${item.visibility} | ${item.maxParticipants} max',
              description:
                  item.description.isEmpty
                      ? 'Hosted competition backed by hosted-competition routes.'
                      : item.description,
              path: '/competitions/${family.pathSegment}/${item.id}',
              tone: GtexSurfaceTone.info,
            ),
    ...
    if (cards.isEmpty) {
      return GteStatePanel(
        title: '${family.label} unavailable',
        message:
            'No backend rows were returned for this competition family yet.',
      ...
      data: (GtexCompetitionDetailBundle value) {
        ...
        return _DetailCard(
          ...
          metrics: <String>[
            ...
            value.standingsFeed.countLabel('Standings'),
            value.fixturesFeed.countLabel('Fixtures'),
            if (value.standingsFeed.message != null)
              'Standings route: ${value.standingsFeed.message}',
            if (value.fixturesFeed.message != null)
              'Fixtures route: ${value.fixturesFeed.message}',
      ...
      data: (HostedCompetitionDetailBundle value) {
        ...
        return _DetailCard(
          ...
          metrics: <String>[
            ...
            value.standingsFeed.countLabel('Standings'),
            value.finance.settlementReadiness.displayLabel,
            if (value.finance.settlementReadiness.missingData.isNotEmpty)
              'Readiness missing ${value.finance.settlementReadiness.missingData.join(', ')}',
            if (value.standingsFeed.message != null)
              'Standings route: ${value.standingsFeed.message}',
