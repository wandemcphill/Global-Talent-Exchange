...
const String _legacy3dMonetizationQuarantinedMessage =
    'Legacy 3D monetization is quarantined; use the backend-authoritative 2D broadcast match center.';
...
class Match3dMatchContext {
  ...
  bool get auto3dEligible =>
      kGtexLegacy3dRuntimeEnabled && (isFinal || isMajorMatch);
...
class Match3dMonetizationService extends ChangeNotifier {
  ...
  void selectRenderMode(RenderMode mode) {
    final RenderMode canonicalMode =
        !kGtexLegacy3dRuntimeEnabled && mode == RenderMode.threeD
            ? RenderMode.twoD
            : mode;
    if (_selectedRenderMode == canonicalMode) {
      return;
    }
    _selectedRenderMode = canonicalMode;
    notifyListeners();
  }
  ...
  Future<Match3dActionResult> claimRewardedAd({
    ...
    if (!kGtexLegacy3dRuntimeEnabled) {
      return const Match3dActionResult.failure(
        message: _legacy3dMonetizationQuarantinedMessage,
  ...
  Future<Match3dActionResult> unlockThreeDForMatch(
    ...
    if (!kGtexLegacy3dRuntimeEnabled) {
      return const Match3dActionResult.failure(
        message:
            'Legacy immersive match view is quarantined; use the 2D broadcast match center.',
        failureReason: Match3dFailureReason.unavailable,
      );
    }
    final Match3dActionResult result = await _processPurchase(
      ...
      successMessage: 'Immersive match view unlocked for this session.',
    );
    if (result.success) {
  ...
  Future<Match3dActionResult> unlockInteraction(
    ...
    switch (interaction) {
      ...
      case Match3dPaidInteraction.alternateCameraAngle:
        ...
        successMessage = 'Alternate camera unlocked for this match.';
