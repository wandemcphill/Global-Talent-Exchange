...
class LiveMatchSpeedMode {
  ...
  factory LiveMatchSpeedMode.fromJson(Object? value) {
    final Map<String, Object?> json =
        GteJson.map(value, label: 'live speed mode');
    return LiveMatchSpeedMode(
      key: GteJson.string(json, <String>['key']),
      label: GteJson.string(json, <String>['label']),
      targetDurationSeconds: GteJson.integer(
        json,
        <String>['target_duration_seconds', 'targetDurationSeconds'],
        fallback: 90,
      ),
    );
  }
...
class LiveMatchSpectateSession {
  ...
  factory LiveMatchSpectateSession.fromJson(Object? value) {
    final Map<String, Object?> json =
        GteJson.map(value, label: 'live spectate session');
    return LiveMatchSpectateSession(
      ...
      websocketPath: GteJson.string(
        json,
        <String>['websocket_path', 'websocketPath'],
      ),
      commentaryWebsocketPath: GteJson.stringOrNull(
        json,
        <String>['commentary_websocket_path', 'commentaryWebsocketPath'],
      ),
      presenceChannel: GteJson.stringOrNull(
        json,
        <String>['presence_channel', 'presenceChannel'],
      ),
      presenceWebsocketPath: GteJson.stringOrNull(
        json,
        <String>['presence_websocket_path', 'presenceWebsocketPath'],
      ),
      replayRoute: GteJson.stringOrNull(
        json,
        <String>['replay_route', 'replayRoute'],
      ),
      speedModes: GteJson.typedList(
        json,
        <String>['speed_modes', 'speedModes'],
