    ...
    final List<String> lineupChanges = _uniqueStrings(<String>[
      ...
      ..._eventLines(
        viewState.events.where(
          (MatchEvent event) => event.type == MatchViewerEventType.substitution,
        ),
      ),
    ]);
    final List<String> injuries = _eventLines(
      viewState.events.where(
        (MatchEvent event) => event.type == MatchViewerEventType.injury,
      ),
    );
    final List<String> suspensions = _eventLines(
      viewState.events.where(
        (MatchEvent event) => event.type == MatchViewerEventType.redCard,
      ),
    );
