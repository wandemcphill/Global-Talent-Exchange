...
abstract class ReplayArchiveRouteRepository {
  ...
  factory ReplayArchiveRouteRepository.standard({
    ...
    if (resolvedMode == GteBackendMode.fixture) {
      return const FixtureReplayArchiveRouteRepository();
    }
...
class FixtureReplayArchiveRouteRepository
    implements ReplayArchiveRouteRepository {
  const FixtureReplayArchiveRouteRepository();

  @override
  Future<ReplayArchiveOverview> loadOverview({
    required bool isAuthenticated,
  }) async {
    final ReplayArchiveEntry publicFinal = ReplayArchiveEntry(
      replayId: 'replay-final-2030',
      fixtureId: 'fixture-final-2030',
      homeClubName: 'Lagos Stars',
      awayClubName: 'Abuja City',
      homeGoals: 3,
      awayGoals: 2,
      competitionName: 'GTEX Super Cup',
      scheduledStart: DateTime.utc(2030, 7, 12, 19),
      finalWhistleAt: DateTime.utc(2030, 7, 12, 21),
      live: false,
      isFinal: true,
      resolvedVisibility: 'public',
      stageName: 'Final',
    );
    final ReplayArchiveEntry personalReplay = ReplayArchiveEntry(
      replayId: 'replay-club-qualifier',
      fixtureId: 'fixture-club-qualifier',
      homeClubName: 'Your Club',
      awayClubName: 'Port Harcourt Wave',
      homeGoals: 1,
      awayGoals: 0,
      competitionName: 'Creator League',
      scheduledStart: DateTime.utc(2030, 7, 5, 18),
      finalWhistleAt: DateTime.utc(2030, 7, 5, 20),
      live: false,
      isFinal: false,
      resolvedVisibility: 'competition',
      stageName: 'Quarterfinal',
    );
    return ReplayArchiveOverview(
      publicFeatured: <ReplayArchiveEntry>[publicFinal],
      myReplays:
          isAuthenticated
              ? <ReplayArchiveEntry>[personalReplay]
              : const <ReplayArchiveEntry>[],
      personalLaneMessage:
          isAuthenticated
              ? null
