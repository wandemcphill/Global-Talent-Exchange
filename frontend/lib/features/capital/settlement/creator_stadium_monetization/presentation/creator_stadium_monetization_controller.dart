...
class CreatorStadiumMonetizationController extends ChangeNotifier {
  ...
  Future<void> loadClubStadium(String clubId, String seasonId) async {
    ...
    try {
      final CreatorStadiumMonetization result = await _repository
          .fetchClubStadium(
            clubId,
  ...
  Future<void> purchaseBroadcast(
    ...
    notifyListeners();
    try {
      latestBroadcastPurchase = await _repository.purchaseMatchBroadcast(
        matchId,
        request,
      );
      await loadMatch(
  ...
  Future<void> sendGift(String matchId, CreatorMatchGiftRequest request) async {
    if (isSendingGift) {
      return;
    }
    isSendingGift = true;
  ...
  Future<void> updateStadiumLevel(
    ...
    notifyListeners();
    try {
      latestStadiumProfile = await _repository.updateStadiumLevel(
        clubId,
        request,
      );
