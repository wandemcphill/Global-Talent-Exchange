  ...
  Future<void> _updateControl() async {
    await showGteFormSheet(
      ...
      onSubmit: (Map<String, String> values) async {
        final int? maxSharesPerClub = int.tryParse(
          values['maxSharesPerClub'] ?? '',
        );
        final int? maxSharesPerFan = int.tryParse(
          values['maxSharesPerFan'] ?? '',
        );
        final int? revenueBps = int.tryParse(values['revenueBps'] ?? '');
        final double? maxPurchaseValue = double.tryParse(
          values['maxPurchaseValue'] ?? '',
        );
                          ...
                          children: <Widget>[
                            ...
                            if (control != null)
                              GteMetricChip(
                                label: 'Revenue bps',
                                value:
                                    control.shareholderRevenueShareBps
                                        .toString(),
                              ),
                          ],
                        ),
