...
import 'package:gte_frontend/features/capital/disputes/data/capital_dispute_api.dart';
...
class _GteDisputeHubScreenState extends State<GteDisputeHubScreen> {
  ...
  void initState() {
    super.initState();
    _disputesFuture = widget.controller.disputeApi.listDisputes();
  }
          ...
          if (snapshot.hasError && !snapshot.hasData) {
            return Center(
              child: GteStatePanel(
                ...
                message: 'We could not sync support disputes from the backend.',
                ...
                return GteSurfacePanel(
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      const SizedBox(height: 6),
                      Text(
                        'Audit reference: ${dispute.reference}/${dispute.resourceId}',
                        style: Theme.of(context).textTheme.bodySmall,
                      ),
                      const SizedBox(height: 6),
                      Text(
...
class _GteDisputeCreateScreenState extends State<GteDisputeCreateScreen> {
  ...
  Future<void> _loadWhatsapp() async {
    try {
      final GteTreasurySettings settings =
          await widget.controller.disputeApi.fetchTreasurySettings();
      if (!mounted) {
        return;
      }
      setState(() {
  ...
  Future<void> _pickAttachment() async {
    ...
    try {
      ...
      final GteAttachment attachment = await widget.controller.disputeApi
          .uploadAttachment(
            file.name,
            bytes,
  ...
  Future<void> _submit() async {
    ...
    try {
      final GteDispute dispute = await widget.controller.disputeApi.openDispute(
        ...
        MaterialPageRoute<void>(
          builder:
              (BuildContext context) => GteDisputeThreadScreen(
                disputeApi: widget.controller.disputeApi,
                disputeId: dispute.id,
              ),
        ),
      );
...
class GteDisputeThreadScreen extends StatefulWidget {
  const GteDisputeThreadScreen({
    ...
    required this.disputeApi,
    this.isAdmin = false,
  });

  final CapitalDisputeApi disputeApi;
...
class _GteDisputeThreadScreenState extends State<GteDisputeThreadScreen> {
  ...
  Future<GteDispute> _loadDispute() async {
    if (widget.isAdmin) {
      return widget.disputeApi.fetchAdminDispute(widget.disputeId);
    }
    return widget.disputeApi.fetchDispute(widget.disputeId);
  }
  ...
  Future<void> _loadWhatsapp() async {
    try {
      final GteTreasurySettings settings =
          await widget.disputeApi.fetchTreasurySettings();
      if (!mounted) {
        return;
      }
      setState(() {
  ...
  Future<void> _pickAttachment() async {
    ...
    try {
      ...
      final GteAttachment attachment = await widget.disputeApi.uploadAttachment(
        file.name,
        bytes,
        contentType: _contentTypeForAttachment(file),
      );
  ...
  Future<void> _sendMessage(GteDispute dispute) async {
    ...
    try {
      ...
      if (widget.isAdmin) {
        await widget.disputeApi.adminSendDisputeMessage(dispute.id, request);
      } else {
        await widget.disputeApi.sendDisputeMessage(dispute.id, request);
      }
      if (!mounted) {
        return;
      }
          ...
          if (snapshot.hasError && !snapshot.hasData) {
            return Center(
              child: GteStatePanel(
                ...
                message: 'We could not sync this support thread.',
                icon: Icons.sync_problem_outlined,
                actionLabel: 'Retry',
                onAction: _refresh,
              ),
