...
double? _officialValueCredits(JsonMap raw) {
  final Object? directValue =
      raw['latest_value_credits'] ?? raw['current_value_credits'];
  if (directValue != null) {
    return numberValue(directValue);
  }
  final JsonMap? marketSnapshot = jsonMapOrNull(raw['latest_market_snapshot']);
  final Object? nestedValue = marketSnapshot?['current_value_credits'];
  if (nestedValue == null) {
    return null;
  }
  return numberValue(nestedValue);
}
