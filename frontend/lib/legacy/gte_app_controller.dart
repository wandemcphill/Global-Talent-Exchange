...
class GteAppController extends ChangeNotifier {
  ...
  bool isLoadingTraderData = false;
  ...
  Future<void> bootstrap() async {
    ...
    final int requestId = _bootstrapGate.begin();
    ...
    try {
      ...
      marketPulse = _decorateMarketPulse(payload[1] as MarketPulse);
    } catch (error) {
      if (_bootstrapGate.isActive(requestId)) {
  ...
  Future<void> loadTraderData() async {
    if (isLoadingTraderData) {
      return;
    }

    final int requestId = _traderGate.begin();
    isLoadingTraderData = true;
    notifyListeners();
    ...
    try {
      ...
      orders = payload[4] as GteOrderListView;
    } catch (error) {
      if (_traderGate.isActive(requestId)) {
        errorMessage = error.toString();
      }
  ...
  void toggleFollow(String playerId) {
    _updatePlayer(
      ...
      (PlayerSnapshot player) =>
          player.copyWith(isFollowed: !player.isFollowed),
    );
  }
