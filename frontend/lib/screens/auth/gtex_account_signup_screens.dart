...
final String _totpSetupAlphabet = String.fromCharCodes(<int>[
  ...
  for (var code = 52; code <= 57; code += 1) code,
]);
...
class GtexAccountSelectorScreen extends StatelessWidget {
  const GtexAccountSelectorScreen({
    super.key,
                  ...
                  LayoutBuilder(
                    builder: (_, c) {
                      ...
                      return GridView.builder(
                        ...
                        itemBuilder: (_, i) => _AccountRoleCard(
                          role: roles[i],
                        ),
                      );
                    },
...
class _GtexUserSignupScreenState extends State<GtexUserSignupScreen> {
  ...
  Widget build(BuildContext context) => _SignupScaffold(
        ...
        children: <Widget>[
          ...
          if (_footballIdentity == 'player' || _footballIdentity == 'both') ...<Widget>[
      ...
      final session = await client.signupUser(
        GteUserSignupRequest(
          ...
          position: _footballIdentity == 'player' || _footballIdentity == 'both'
              ? _position.text
              : null,
          dominantFoot: _footballIdentity == 'player' || _footballIdentity == 'both'
              ? _dominantFoot.text
              : null,
          heightCm: _footballIdentity == 'player' || _footballIdentity == 'both'
              ? int.tryParse(_height.text)
              : null,
          jerseyNumber: _footballIdentity == 'player' || _footballIdentity == 'both'
              ? int.tryParse(_jersey.text)
              : null,
          preferredRole: _footballIdentity == 'player' || _footballIdentity == 'both'
              ? _preferredRole.text
              : null,
        ),
      );
...
class GtexCreatorSignupScreen extends StatefulWidget {
  ...
  @override
  State<GtexCreatorSignupScreen> createState() => _GtexCreatorSignupScreenState();
}
...
class _GtexTraderSignupScreenState extends State<GtexTraderSignupScreen> {
  ...
  Widget build(BuildContext context) => _SignupScaffold(
        title: 'Trader',
        subtitle: 'Enter the GTEX economy with dedicated trading and wallet security.',
                            ...
                            Row(
                              children: <Widget>[
                                Container(
                                  ...
                                  height: 48,
                                  decoration: BoxDecoration(
                                    color: const Color(0xFFB9FF3D).withValues(alpha: .1),
                                    borderRadius: BorderRadius.circular(14),
                                  ),
