...
class GteTransferRoomScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<TransferRoomEntry> entries = pulse?.transferRoom ?? const <TransferRoomEntry>[];
                ...
                Padding(
                  ...
                  child: GteSurfacePanel(
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        Text('Live acquisition signal', style: Theme.of(context).textTheme.headlineSmall),
                  ...
                  child: TabBarView(
                    children: <Widget>[
                      ...
                      _RoomLane(
                        ...
                        players: controller.featuredPlayers.take(3).toList(growable: false),
                        onOpenPlayer: onOpenPlayer,
                      ),
                    ],
                  ),
      ...
      children: <Widget>[
        GteSurfacePanel(
          child: Column(
            ...
            children: <Widget>[
              ...
              if (entries.isEmpty)
                Text('No entries in this lane yet.', style: Theme.of(context).textTheme.bodyMedium)
              else
                ...entries.map(
                  (TransferRoomEntry entry) => Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: <Widget>[
                        Text(entry.headline, style: Theme.of(context).textTheme.titleMedium),
                        const SizedBox(height: 4),
                        Text(entry.activity, style: Theme.of(context).textTheme.bodyMedium),
                        const SizedBox(height: 4),
                        Text('${entry.marketCredits} cr', style: Theme.of(context).textTheme.bodyLarge),
                      ],
                    ),
                  ),
                ),
        ...
        GteSurfacePanel(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: <Widget>[
              Text('Suggested player routes', style: Theme.of(context).textTheme.titleLarge),
              const SizedBox(height: 14),
              if (players.isEmpty)
                Text('No player routes available.', style: Theme.of(context).textTheme.bodyMedium)
              else
                ...players.map(
                  (PlayerSnapshot player) => ListTile(
                    contentPadding: EdgeInsets.zero,
                    title: Text(player.name),
                    subtitle: Text('${player.club} - ${player.marketCredits} cr'),
...
List<TransferRoomEntry> _entriesForLane(List<TransferRoomEntry> entries, String lane) {
  return entries
