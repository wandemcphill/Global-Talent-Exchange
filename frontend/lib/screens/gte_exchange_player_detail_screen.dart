...
class _GteExchangePlayerDetailScreenState
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    const GtexSectionHeader(
                      ...
                      title:
                          'Read the quote, the history, and the visible depth together.',
                      description:
                          'The market lane stays compact but explicit. Sparse history, one-sided books, and indicative fills are called out instead of hidden.',
                      accent: GteShellTheme.accent,
                    ),
                    const SizedBox(height: 14),
  ...
  Widget _buildHero(
    ...
    final avatar = AvatarMapper.fromMarketIdentity(
      identity,
          ...
          Wrap(
            ...
            children: <Widget>[
              ...
              GteMetricChip(
                label: 'Trust',
                value:
                    marketProfile.tradeTrustScore?.toStringAsFixed(1) ?? '--',
              ),
              GteMetricChip(
                label: 'Reference',
  ...
  Widget _buildLifecycleSurface(
    ...
    final GtePlayerAgencySummary? agency = lifecycle.agencySummary;
    final String? reasonSnippet =
        agency?.reasonSnippet ?? lifecycle.transferStatus.reason;
    return GteSurfacePanel(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          ...
          Wrap(
            ...
            children: <Widget>[
              ...
              GteMetricChip(
                label: 'Transfer',
                value:
                    lifecycle.transferStatus.eligible ? 'Eligible' : 'Locked',
                positive: lifecycle.transferStatus.eligible,
              ),
              if (lifecycle.transferStatus.windowLabel != null)
                GteMetricChip(
                  label: 'Window',
                  value: lifecycle.transferStatus.windowLabel!,
                ),
              if (lifecycle.transferStatus.lastBidStatus != null)
                GteMetricChip(
                  label: 'Bid flow',
                  value: lifecycle.transferStatus.lastBidStatus!,
                ),
            ],
          ),
          if (agency != null || reasonSnippet != null) ...<Widget>[
            const SizedBox(height: 16),
            Text('Agency summary',
              ...
              children: <Widget>[
                if (agency != null)
                  GteMetricChip(
                    ...
                    value: agency.transferStanceLabel,
                  ),
                if (agency != null)
                  GteMetricChip(
                    label: 'Contract stance',
                    value: agency.contractStanceLabel,
                  ),
                if (agency?.moraleLabel != null)
                  GteMetricChip(
                    label: 'Morale',
                    value: agency!.moraleLabel!,
                  ),
              ],
            ),
            if (reasonSnippet != null) ...<Widget>[
