...
class ManagerMarketRepository {
  ...
  Future<Map<String, dynamic>> fetchFilters() async =>
      _getMap('/api/managers/filters', auth: false);
  ...
  Future<Map<String, dynamic>> fetchTeam() async =>
      _getMap('/api/managers/team');
  Future<List<Map<String, dynamic>>> fetchListings() async =>
      _getList('/api/managers/trade-listings', auth: false);
  Future<List<Map<String, dynamic>>> fetchMyListings() async =>
      _getList('/api/managers/my-trade-listings');
  Future<Map<String, dynamic>> fetchRecommendation() async =>
      _getMap('/api/managers/recommendation');
  Future<List<Map<String, dynamic>>> fetchTradeHistory({
    String? managerId,
    int limit = 50,
  }) async => _getList(
    '/api/managers/history',
    query: <String, Object?>{
      'limit': limit,
      if (managerId != null && managerId.isNotEmpty) 'manager_id': managerId,
    },
  );
  Future<Map<String, dynamic>> compareManagers(
    String leftManagerId,
    String rightManagerId,
  ) async => _getMap(
    '/api/managers/compare',
    query: <String, Object?>{
      'left_manager_id': leftManagerId,
      'right_manager_id': rightManagerId,
    },
    auth: false,
  );

  Future<void> recruit(String managerId, {String slot = 'bench'}) => _post(
    '/api/managers/recruit',
    body: <String, Object?>{'manager_id': managerId, 'slot': slot},
  );
  Future<void> assign(String assetId, String slot) => _post(
    '/api/managers/assign',
    body: <String, Object?>{'asset_id': assetId, 'slot': slot},
  );
  Future<void> release(String assetId) =>
      _post('/api/managers/$assetId/release');
  Future<void> createListing(String assetId, String askingPriceCredits) =>
      _post(
        '/api/managers/trade-listings',
        body: <String, Object?>{
          'asset_id': assetId,
          'asking_price_credits': askingPriceCredits,
        },
      );
  Future<void> buyListing(String listingId) =>
      _post('/api/managers/trade-listings/$listingId/buy');
  Future<void> cancelListing(String listingId) =>
      _post('/api/managers/trade-listings/$listingId/cancel');
  Future<void> swap(
    String proposerAssetId,
    String requestedAssetId,
    String cashAdjustmentCredits,
  ) => _post(
    '/api/managers/swap',
    body: <String, Object?>{
      'proposer_asset_id': proposerAssetId,
      'requested_asset_id': requestedAssetId,
      'cash_adjustment_credits': cashAdjustmentCredits,
    },
  );
  ...
  Future<Map<String, dynamic>> _getMap(
    ...
    throw const GteApiException(
      ...
      message: 'Unexpected manager response shape.',
    );
  }
  ...
  Future<List<Map<String, dynamic>>> _getList(
    ...
    throw const GteApiException(
      type: GteApiErrorType.parsing,
      message: 'Unexpected manager list response shape.',
    );
  }
