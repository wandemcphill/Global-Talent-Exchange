...
void main() {
  test('mock api exposes seeded players and market pulse', () async {
    final GteMockApi api = GteMockApi.capitalFixtures(latency: Duration.zero);
  ...
  test(
    ...
    () async {
      ...
      final GteOrderBook bookAfterPlace = await api.fetchOrderBook(
        'jude-bellingham',
      );
      ...
      expect(
        openOrders.items.any((GteOrderRecord item) => item.id == order.id),
        isTrue,
      );
      expect(
      ...
      expect(
        reservedWallet.reservedBalance,
        greaterThan(startingWallet.reservedBalance),
      );
      expect(
        bookAfterPlace.bids.any(
          (GteOrderBookLevel level) => level.price == order.maxPrice,
        ),
        isTrue,
      );
      expect(cancelled.status, GteOrderStatus.cancelled);
      expect(
        restoredWallet.availableBalance,
        closeTo(startingWallet.availableBalance, 0.001),
      );
      expect(
        restoredWallet.reservedBalance,
