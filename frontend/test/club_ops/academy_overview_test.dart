...
void main() {
  testWidgets(
    'academy overview shows pathway summary and opens player detail',
    (WidgetTester tester) async {
      ...
      await tester.scrollUntilVisible(find.text('Recent promotions'), 300);
      expect(find.text('Recent promotions'), findsOneWidget);
    },
  );

  testWidgets(
    'academy player rows handle long mixed metadata without overflow',
    (WidgetTester tester) async {
      const AcademyPlayer player = AcademyPlayer(
        id: 'academy-long-name',
        playerId: 'player-long-name',
        name:
            'Very Long Canonical Real Player Name With Regen Context Attached',
      ...
      await tester.pumpWidget(
        MaterialApp(
          theme: GteShellTheme.build(),
          home: Scaffold(
            body: SizedBox(width: 280, child: AcademyPlayerRow(player: player)),
          ),
        ),
      );
      await tester.pumpAndSettle();
      ...
      expect(
        find.textContaining('Very Long Canonical Real Player Name'),
        findsOneWidget,
      );
