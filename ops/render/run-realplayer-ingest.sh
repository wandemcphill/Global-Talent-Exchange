...
#   BACKOFF_START              initial retry backoff seconds (default 30)
#   BACKOFF_MAX                maximum retry backoff seconds (default 120)
...
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/gtex-realplayer-ingest.XXXXXX")"
trap 'rm -f "$RUN_LOG"' EXIT

# Scripts may include connection strings or provider credentials in exception
# payloads. Never stream raw script output into Render logs. Keep exit status
# unchanged while redacting connection URLs and token-like query parameters.
run_redacted() {
  : > "$RUN_LOG"
  "$@" >"$RUN_LOG" 2>&1
  local code=$?
...
run_market_backfill() {
  ...
  run_redacted python -u scripts/backfill_real_league_share_markets.py \
...
while [ "$attempt" -lt "$MAX_ATTEMPTS" ]; do
  ...
  run_redacted python -u scripts/refresh_target_league_real_players.py \
    --database-url "$DATABASE_URL" \
