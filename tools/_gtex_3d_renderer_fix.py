...
p = ROOT/'Gtex_Test_Migration/Assets/Code/GTEX/GtexMatchRuntime.cs'; t=p.read_text(encoding='utf-8-sig')
...
t=replace_method(t,'private void DriveBall(','''        private void DriveBall()
        {
            ...
            {
                ClearSyntheticBallTransit(); hasFilteredBallTarget = false; runtimeTraceBallSpeed = 0f; return;
            }
            var target = ClampToFieldBounds(ConvertIncomingPlaybackPosition(currentState.ballPosition, currentState).ClampedWorld, true);
            var velocity = ResolvePlaybackBallVelocity(currentState.ballPosition, false); velocity.y = 0f;
...
'''
if marker not in t: raise SystemExit('missing ProcessBehaviours')
section=t[t.find(marker):t.find(marker)+900]
if 'GtexRuntimeState.ActiveMode == GtexRuntimeMode.LivePlayback' not in section: t=t.replace(marker,guard,1)
write(p,t)

p=ROOT/'Gtex_Test_Migration/Assets/Code/MatchEngine/Players/PlayerController/CodeBasedController.cs'; t=p.read_text(encoding='utf-8-sig')
...
if 'private Vector3 externalPlaybackPresentationVelocity;' not in t: t=t.replace('        private float nextExternalPlaybackValidationAt;\n','        private float nextExternalPlaybackValidationAt;\n        private Vector3 externalPlaybackPresentationVelocity;\n',1)
...
new='''            if (GtexRuntimeState.ActiveMode == GtexRuntimeMode.LivePlayback)
            ...
            hasExternalPlaybackTarget = true;'''
if old not in t: raise SystemExit('missing Ball state tail')
...
if needle not in t: raise SystemExit('missing camera behavior call')
...
if 'private void ApplyGtexBroadcastCamera(' not in t:
    ...
    if idx<0: raise SystemExit('camera class end missing')
    t=t[:idx]+'    }'+helper+'\n}'
write(p,t)

# cleanup failed temporary renderer/finalizer workflows that should not ship.
for rel in [
    '.github/workflows/gtex-3d-final-repair.yml',
    '.github/workflows/gtex-main-finalize.yml',
    '.github/workflows/gtex-authoritative-finalize.yml',
    '.github/workflows/gtex-3d-renderer-repair.yml',
    '.github/workflows/gtex-3d-pr-finalize.yml',
    'tools/.gtex-renderer-repair-trigger',
    'tools/.gtex-pr-finalizer-trigger',
]:
    q=ROOT/rel
...
assert 'TryDriveSyntheticBallTransit' not in method(r, 'private void DriveBall()')
