...
class InstagramPublisher:
    ...
    @classmethod
    def from_env(cls, environ: Mapping[str, str] | None = None) -> "InstagramPublisher":
        env = environ or os.environ
        return cls(
            endpoint_url=env.get("GTE_PUBLISHER_INSTAGRAM_API_URL"),
            access_token=env.get("GTE_PUBLISHER_INSTAGRAM_ACCESS_TOKEN"),
            timeout_seconds=max(int(env.get("GTE_PUBLISHER_INSTAGRAM_TIMEOUT_SECONDS", "20")), 1),
    ...
    def publish(
        ...
        *,
        ...
        payload = {
            ...
            "media_type": "REELS",
            "share_to_feed": bool((metadata or {}).get("share_to_feed", True)),
            "collaborators": list((metadata or {}).get("collaborators") or []),
        }
        ...
        if not self.access_token:
            raise RuntimeError("Instagram publisher access token is not configured.")
        ...
        post_id = data.get("post_id") or data.get("creation_id") or data.get("id") or _dry_run_post_id(
            self.platform_name,
            payload,
        )
        return {
