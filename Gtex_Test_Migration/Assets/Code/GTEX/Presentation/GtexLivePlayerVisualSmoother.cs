...
{
    /// <summary>
    /// Final render-stage smoothing for authoritative live playback.
    /// The GTEX runtime remains authoritative; this component only converts
    /// discontinuous network samples into continuous visible motion.
    /// </summary>
    ...
    public sealed class GtexLivePlayerVisualSmoother : MonoBehaviour
    {
        ...
        private readonly Dictionary<CodeBasedController, VisualState> states =
            new Dictionary<CodeBasedController, VisualState>();
        ...
        private void LateUpdate()
        {
            ...
            for (var index = 0; index < players.Length; index++)
            {
                ...
                if (player == null || !player.isActiveAndEnabled || player.UnityObject == null)
                {
                    continue;
                }
            ...
            {
                if (!seen.Contains(pair.Key) || pair.Key == null)
                {
                    stale.Add(pair.Key);
                }
            }
        ...
        private void SmoothPlayer(CodeBasedController player, float dt)
        {
            ...
            if (targetDistance > 3f)
            {
                maxSpeed = Mathf.Min(
                    MaxVisualSpeed,
                    Mathf.Max(4.5f, targetDistance / PositionSmoothTime * 0.34f));
            }
            ...
            var lookDirection = movement.sqrMagnitude > 0.0001f
                ? movement.normalized
            ...
            if (lookDirection.sqrMagnitude > 0.0001f)
            {
                ...
                var angle = Quaternion.Angle(state.rotation, targetRotation);
                if (angle > 1f)
                {
                    state.rotation = Quaternion.RotateTowards(
                        state.rotation,
                        targetRotation,
                        MaxVisualTurnDegreesPerSecond * dt);
                }
            }
            ...
            if (animator != null)
            {
                ...
                animator.SetFloat(
                    PlayerAnimatorVariable.MoveSpeed,
                    movementSpeed <= IdleSpeed ? 0f : Mathf.Lerp(WalkSpeed, 1f, normalizedSpeed));
            }
