...
namespace FStudio.GTEX.Presentation
{
    ...
    public sealed class GtexLiveBroadcastCameraDriver : MonoBehaviour
    {
        ...
        {
            if (!GtexRuntimeState.IsStarted || GtexRuntimeState.ActiveMode != GtexRuntimeMode.LivePlayback) return;
            ResolveSceneReferences();
            if (!initialized || targetCamera == null || ball == null) return;
        ...
        {
            if (targetCamera == null) targetCamera = Camera.main ?? FindFirstObjectByType<Camera>();
            if (ball == null) ball = Ball.Current ?? FindFirstObjectByType<Ball>();
            if (legacyCameraSystem == null) legacyCameraSystem = FindFirstObjectByType<CameraSystem>();
            initialized = targetCamera != null && ball != null;
        }

        private void DisableLegacyCameraController()
        {
            if (legacyCameraSystem != null && legacyCameraSystem.enabled) legacyCameraSystem.enabled = false;
        }

        private GtexPitchSpace ResolvePitchSpace()
        {
            var manager = MatchManager.Current;
            if (manager != null && manager.ExternalPlaybackPitchSpace != null) return manager.ExternalPlaybackPitchSpace;
        ...
        private void ApplyBroadcastFrame(float deltaTime)
        {
            ...
            var playDirection = velocity.sqrMagnitude > 0.16f ? velocity.normalized : lastPlayDirection;
            playDirection.y = 0f;
            if (playDirection.sqrMagnitude <= 0.0001f) playDirection = Vector3.right;
            else playDirection.Normalize();
            lastPlayDirection = Vector3.Slerp(lastPlayDirection, playDirection, 1f - Mathf.Exp(-5.5f * Mathf.Max(deltaTime, 0.001f)));
            ...
            {
                focus = Vector3.SmoothDamp(focus, desiredFocus, ref focusVelocity, FollowSmoothTime, Mathf.Infinity, Mathf.Max(deltaTime, 1f / 120f));
            }

            var lateral = new Vector3(-lastPlayDirection.z, 0f, lastPlayDirection.x);
            if (lateral.sqrMagnitude <= 0.0001f) lateral = Vector3.back;
            lateral.Normalize();
            lateral *= CameraSideSign;

            var desiredPosition = focus - lastPlayDirection * PlayOffset + lateral * LateralOffset + Vector3.up * CameraHeight;
            var cameraXMargin = Mathf.Clamp(pitch.Length * 0.08f, 7f, 11f);
            var cameraZMargin = Mathf.Clamp(pitch.Width * 0.08f, 5f, 8f);
            desiredPosition.x = Mathf.Clamp(desiredPosition.x, pitch.MinX - cameraXMargin, pitch.MaxX + cameraXMargin);
            desiredPosition.z = Mathf.Clamp(desiredPosition.z, pitch.MinZ - LateralOffset - cameraZMargin, pitch.MaxZ + LateralOffset + cameraZMargin);
