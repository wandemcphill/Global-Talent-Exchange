...
#pragma warning restore 0109
        ...
        {
            // During GTEX live playback, GtexMatchRuntime already resolves and
            // interpolates the authoritative pose once per rendered frame. The
            // legacy FixedUpdate chase must not run a second movement solver.
        ...
        public void SetExternalPlaybackPose(Vector3 position, Quaternion rotation, bool snap = false)
        {
            ...
            // Apply the pose supplied by GTEX exactly once. There is no implicit
            // distance-triggered teleport here; only an explicit GTEX snap may snap.
        ...
        private (float turnResult, float angleDifferency) AgileToDirection(Vector3 targetDirection)
        {
            ...
            var currentDirection = Direction; currentDirection.y = 0f;
        ...
        {
            var forward = transform.forward; forward.y = 0f; if (forward.sqrMagnitude <= 0.0001f) forward = Vector3.forward; else forward.Normalize();
            var desiredDirection = Direction; desiredDirection.y = 0f; if (desiredDirection.sqrMagnitude <= 0.0001f) { targetAnimatorDirection = Vector2.zero; return; }
            desiredDirection.Normalize(); var angle = Vector3.SignedAngle(forward, desiredDirection, Vector3.up);
        ...
        public bool MoveTo(in float deltaTime, Vector3 targetPosition, bool faceTowards = true, MovementType movementType = MovementType.BestHeCanDo)
        {
            ...
            this.targetPosition = targetPosition; targetPosition.y = ResolveGroundedY(); var distance = Vector3.Distance(Position, targetPosition); var targetDirection = targetPosition - Position; targetDirection.y = 0f; var reachRadius = collider != null ? collider.radius : 0.35f;
            if (distance > reachRadius)
            {
                if (targetDirection.sqrMagnitude <= 0.0001f) { targetDirection = transform.forward; targetDirection.y = 0f; }
                targetDirection.Normalize(); var newRotation = Quaternion.LookRotation(targetDirection); var directionAgile = AgileToDirection(targetDirection);
                if (Direction.sqrMagnitude > 0.0001f) { var currentRotation = Quaternion.LookRotation(Direction.normalized); Direction = Quaternion.Slerp(currentRotation, newRotation, Mathf.Max(deltaTime, 0f) * directionAgile.turnResult) * Vector3.forward; } else Direction = newRotation * Vector3.forward;
                var targetMovement = 1f - (directionAgile.angleDifferency / 180f); if (movementType == MovementType.Relax) targetMovement *= 0.25f; else if (movementType == MovementType.Normal) targetMovement *= 0.75f; TargetMoveSpeed = Mathf.Clamp01(targetMovement);
            }
            else { Stop(in deltaTime); return true; }
            if (faceTowards) LookTo(in deltaTime, targetPosition - Position); return false;
        }
        public bool LookTo(in float deltaTime, Vector3 lookDirection)
        {
            if (externalPlaybackEnabled) return false;
            lookDirection.y = 0f; if (lookDirection.sqrMagnitude <= 0.0001f) return true; lookDirection.Normalize(); var agileSpeed = AgileToDirection(lookDirection).turnResult; var holdingBallModifier = BasePlayer != null && BasePlayer.IsHoldingBall ? Mathf.Max(1f, EngineSettings.Current != null ? EngineSettings.Current.AgileToDirectionWhenHoldingBallModifier : 1f) : 1f; LerpRotation(in deltaTime, Quaternion.LookRotation(lookDirection, Vector3.up), agileSpeed * holdingBallModifier); var angle = Vector3.SignedAngle(transform.forward, lookDirection, Vector3.up); var ballHeightMod = 0f; if (BasePlayer != null && BasePlayer.IsHoldingBall && !BasePlayer.IsThrowHolder && Ball.Current != null) ballHeightMod = Ball.Current.transform.position.y * LegacyLookBallHeightApprovalBonus; var approvalAngle = BasePlayer != null && BasePlayer.IsThrowHolder ? 10f : LegacyLookApprovalAngle + ballHeightMod; return Mathf.Abs(angle) <= approvalAngle;
        }
