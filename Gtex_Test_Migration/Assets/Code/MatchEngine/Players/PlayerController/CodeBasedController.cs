...
namespace FStudio.MatchEngine.Players.PlayerController
{
    ...
    public partial class CodeBasedController : MonoBehaviour, IPlayerController
    {
        ...
        public Vector3 Position => transform != null ? transform.position : lastKnownPosition;
        ...
        public bool IsPhysicsEnabled { get => m_IsPhysicsEnabled; set { m_IsPhysicsEnabled = value; if (rigidbody != null && !externalPlaybackEnabled) rigidbody.isKinematic = !value; if (collider != null && !externalPlaybackEnabled) collider.enabled = value; } }
...
#pragma warning restore 0109
        ...
        private void Awake() { rigidbody = GetComponent<Rigidbody>(); collider = GetComponent<CapsuleCollider>(); if (rigidbody != null) { rigidbody.collisionDetectionMode = CollisionDetectionMode.ContinuousSpeculative; rigidbody.interpolation = RigidbodyInterpolation.Interpolate; rigidbody.linearDamping = 1; rigidbody.constraints = RigidbodyConstraints.FreezeRotation | RigidbodyConstraints.FreezePositionY; } lastKnownPosition = transform != null ? transform.position : Vector3.zero; }
        private void FixedUpdate() { if (externalPlaybackEnabled) return; }
        ...
        public void SetInstantPosition(Vector3 position) { position.y = ResolveGroundedY(); transform.position = position; lastKnownPosition = position; if (rigidbody != null) rigidbody.position = position; }
        ...
        public void SetExternalPlayback(bool value) { externalPlaybackEnabled = value; if (value) m_IsPhysicsEnabled = true; if (rigidbody != null) { GtexPlaybackPhysicsUtil.SafeSetRigidbodyVelocity(rigidbody, Vector3.zero, Vector3.zero); rigidbody.isKinematic = value || !m_IsPhysicsEnabled; rigidbody.interpolation = value ? RigidbodyInterpolation.None : RigidbodyInterpolation.Interpolate; } if (collider != null) collider.enabled = value ? false : m_IsPhysicsEnabled; hasExternalPlaybackPose = false; }
        public void SetExternalPlaybackPose(Vector3 position, Quaternion rotation, bool snap = false) { rotation = Quaternion.Euler(0f, rotation.eulerAngles.y, 0f); externalPlaybackTargetPosition = position; externalPlaybackTargetRotation = rotation; hasExternalPlaybackPose = true; if (!externalPlaybackEnabled || rigidbody == null) { SetInstantPosition(position); SetInstantRotation(rotation); return; } GtexPlaybackPhysicsUtil.ApplyExternalPlaybackPosition(transform, rigidbody, position, rotation, snap); lastKnownPosition = transform.position; }
        public void SetPosition(Vector3 position) { position.y = ResolveGroundedY(); lastKnownPosition = position; if (rigidbody != null) rigidbody.position = position; transform.position = position; }
        private static float ResolveGroundedY() { var pitchSpace = MatchManager.Current != null ? MatchManager.Current.ExternalPlaybackPitchSpace : null; return pitchSpace != null ? pitchSpace.GrassY : 0f; }
