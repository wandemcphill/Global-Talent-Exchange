...
namespace FStudio.MatchEngine.Players.PlayerController
{
    ...
    public partial class CodeBasedController : MonoBehaviour, IPlayerController
    {
        ...
        public Vector3 Position
        {
            get
            {
                if (this == null || gameObject == null || !gameObject) return lastKnownPosition;
                if (transform == null) return lastKnownPosition;
...
#pragma warning restore 0109
        ...
        public void SetRotation(Quaternion rotation)
        {
            ...
            if (rigidbody != null) rigidbody.rotation = rotation;
            transform.rotation = rotation;
        }
        public void LerpRotation(in float deltaTime, Quaternion rotation, float agility)
        {
            if (rigidbody == null) return;
            rigidbody.rotation = Quaternion.Slerp(transform.rotation, rotation, deltaTime * agility);
        ...
        private (float turnResult, float angleDifferency) AgileToDirection(Vector3 targetDirection)
        {
            ...
            if (targetDirection.sqrMagnitude <= 0.0001f) return (1f, 0f);
            targetDirection.Normalize();
            var currentDirection = Direction;
            currentDirection.y = 0f;
            if (currentDirection.sqrMagnitude <= 0.0001f) currentDirection = transform.forward;
            currentDirection.y = 0f;
            if (currentDirection.sqrMagnitude <= 0.0001f) currentDirection = targetDirection; else currentDirection.Normalize();
            var angleDifferency = Mathf.Abs(Vector3.SignedAngle(currentDirection, targetDirection, Vector3.up));
            var agility = BasePlayer != null && BasePlayer.MatchPlayer != null ? Mathf.Max(0.25f, BasePlayer.MatchPlayer.GetAgility()) : 4f;
            var settings = EngineSettings.Current;
            var moveHardness = settings != null && settings.AgileToDirectionMoveSpeedHardness != null ? settings.AgileToDirectionMoveSpeedHardness.Evaluate(MoveSpeed) : Mathf.Lerp(0.8f, 1.8f, Mathf.Clamp01(MoveSpeed / 6f));
            var angleHardness = settings != null && settings.AgileToDirectionAngleDifferencyHardness != null ? settings.AgileToDirectionAngleDifferencyHardness.Evaluate(angleDifferency) : Mathf.Lerp(0.6f, 2.25f, Mathf.InverseLerp(0f, 180f, angleDifferency));
        ...
        private void CalculateAnimatorDirection()
        {
            ...
            if (desiredDirection.sqrMagnitude <= 0.0001f) { targetAnimatorDirection = Vector2.zero; return; }
        ...
        public void SetPlayer(int number, PlayerBase basePlayer, Material kitMaterial)
        {
            ...
            if (playerGraphic != null && basePlayer != null && basePlayer.MatchPlayer != null) playerGraphic.SetPlayer(number, kitMaterial, basePlayer.MatchPlayer.Player);
            ApplyBasePlayerPresentation();
            if (shadow == null && ShadowRenderer.Current != null) shadow = ShadowRenderer.Current.Get();
        }
        private void ApplyBasePlayerPresentation()
        {
            if (BasePlayer == null || BasePlayer.MatchPlayer == null || BasePlayer.MatchPlayer.Player == null) return;
            gameObject.name = BasePlayer.MatchPlayer.Player.Name;
            if (playerAnimator != null) playerAnimator.SetFloat(PlayerAnimatorVariable.Agility, BasePlayer.MatchPlayer.ActualAgility / 100f);
            if (UI != null) UI.SetName(BasePlayer.MatchPlayer.Player.Name);
        }
        ...
        {
            if (externalPlaybackEnabled) return false;
            ...
            {
                if (targetDirection.sqrMagnitude <= 0.0001f) { targetDirection = transform.forward; targetDirection.y = 0f; }
                targetDirection.Normalize();
                ...
                if (Direction.sqrMagnitude > 0.0001f)
                {
                    ...
                    Direction = Quaternion.Slerp(currentRotation, newRotation, Mathf.Max(deltaTime, 0f) * directionAgile.turnResult) * Vector3.forward;
                }
                else Direction = newRotation * Vector3.forward;
                var targetMovement = 1f - (directionAgile.angleDifferency / 180f);
                if (movementType == MovementType.Relax) targetMovement *= 0.25f;
                else if (movementType == MovementType.Normal) targetMovement *= 0.75f;
                TargetMoveSpeed = Mathf.Clamp01(targetMovement);
            }
            else { Stop(in deltaTime); return true; }
            if (faceTowards) LookTo(in deltaTime, targetPosition - Position);
            return false;
        }
        public bool LookTo(in float deltaTime, Vector3 lookDirection)
        {
            if (externalPlaybackEnabled) return false;
            lookDirection.y = 0f;
            if (lookDirection.sqrMagnitude <= 0.0001f) return true;
            lookDirection.Normalize();
            var agileSpeed = AgileToDirection(lookDirection).turnResult;
            var holdingBallModifier = BasePlayer != null && BasePlayer.IsHoldingBall ? Mathf.Max(1f, EngineSettings.Current != null ? EngineSettings.Current.AgileToDirectionWhenHoldingBallModifier : 1f) : 1f;
            LerpRotation(in deltaTime, Quaternion.LookRotation(lookDirection, Vector3.up), agileSpeed * holdingBallModifier);
            var angle = Vector3.SignedAngle(transform.forward, lookDirection, Vector3.up);
            var ballHeightMod = 0f;
            if (BasePlayer != null && BasePlayer.IsHoldingBall && !BasePlayer.IsThrowHolder && Ball.Current != null) ballHeightMod = Ball.Current.transform.position.y * LegacyLookBallHeightApprovalBonus;
            var approvalAngle = BasePlayer != null && BasePlayer.IsThrowHolder ? 10f : LegacyLookApprovalAngle + ballHeightMod;
        ...
        public void ProcessMovement(in float time, in float deltaTime)
        {
            ...
            var dribbleModifier = BasePlayer != null && BasePlayer.MatchPlayer != null ? BasePlayer.MatchPlayer.GetDribbleSpeedModifier() : 1f;
            var topSpeed = BasePlayer != null && BasePlayer.MatchPlayer != null ? BasePlayer.MatchPlayer.GetTopSpeed() : 4.5f;
            var finalSpeed = TargetMoveSpeed * topSpeed * (BasePlayer != null && BasePlayer.IsHoldingBall ? dribbleModifier : 1f);
            var shouldMove = finalSpeed > MoveSpeed;
            var acceleration = finalSpeed < MoveSpeed ? 3f : (BasePlayer != null && BasePlayer.MatchPlayer != null ? BasePlayer.MatchPlayer.GetAcceleration() : 6f);
            MoveSpeed = Mathf.Lerp(MoveSpeed, finalSpeed, safeDeltaTime * acceleration);
            var angle = Direction.sqrMagnitude > 0.0001f ? Mathf.Abs(Vector3.SignedAngle(transform.forward, Direction, Vector3.up)) : 0f;
            var movementLean = Mathf.Abs(finalSpeed - MoveSpeed) * LegacyMovementDirectionSpeedLeaningModifier + angle * LegacyMovementDirectionAngleLeaningModifier;
            movementLean /= angle / 90f + 1f;
            var shouldStop = !enabled || MatchManager.Current == null || (!MatchManager.Current.MatchFlags.HasFlag(MatchStatus.Playing) && !MatchManager.Current.MatchFlags.HasFlag(MatchStatus.Special));
            ApplyLegacyLocomotionAnimator(shouldStop ? Vector3.zero : Direction, shouldStop ? 0f : MoveSpeed, animatorLerpSpeed, shouldStop);
        ...
        {
            if (externalPlaybackEnabled || playerAnimator == null || BasePlayer == null || ball == null || !BasePlayer.IsHoldingBall || (!BasePlayer.IsThrowHolder && matchStatus != MatchStatus.Playing)) return;
            var followSpeedMod = 1f;
            if (BasePlayer.ActiveBehaviour is ChipShootingBehaviour || BasePlayer.ActiveBehaviour is ShootingBehaviour || BasePlayer.ActiveBehaviour is PassingBehaviour || BasePlayer.ActiveBehaviour is CrossingBehaviour) followSpeedMod = 1f - ball.transform.position.y;
            followSpeedMod = Mathf.Clamp(followSpeedMod, 0.4f, 1f);
            var ballHolderSituation = PlayerBallPoint.Situation.Normal;
            if (BasePlayer.IsThrowHolder) ballHolderSituation = PlayerBallPoint.Situation.ThrowIn;
            else if (BasePlayer.IsGKUntouchable && !BasePlayer.IsGoalKickHolder) ballHolderSituation = PlayerBallPoint.Situation.GK;
        ...
        {
            if (playerAnimator == null) { result = animatorVariable; return false; }
        ...
        {
            if (transform != null) lastKnownPosition = transform.position;
            if (shadow != null) shadow.position = Position;
        }
        ...
        private void ApplyLegacyLocomotionAnimator(Vector3 worldDirection, float moveSpeed, float animatorLerpSpeed, bool shouldStop)
        {
            ...
            if (shouldStop) targetAnimatorDirection = Vector2.zero;
            if (Mathf.Abs(targetAnimatorDirection.x) < 0.001f) targetAnimatorDirection.x = 0f;
            if (Mathf.Abs(targetAnimatorDirection.y) < 0.001f) targetAnimatorDirection.y = 0f;
