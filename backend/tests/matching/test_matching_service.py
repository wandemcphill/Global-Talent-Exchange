...
from backend.app.matching.service import InvalidOrderTransitionError, MatchingService
...
def _grant_position(session, current_user, *, player_id: str, quantity: Decimal) -> None:
    ...
    wallet_service.credit_position_units(
        ...
        player_id=player_id,
        quantity=quantity,
        reference=f"seed-position-{current_user.id}-{player_id}",
        description="Seed player position for testing",
        external_reference=f"seed-position-{current_user.id}-{player_id}",
    )
    session.commit()
