...
def test_payment_rails_drop_stale_non_live_defaults(tmp_path: Path) -> None:
    ...
    assert [rail["provider"] for rail in rails] == [
...
def test_payment_rail_update_accepts_paystack_provider(tmp_path: Path) -> None:
    with _build_client(tmp_path) as client:
        response = client.put(
            "/api/admin/god-mode/payment-rails",
            json={
                ...
                "reason": "Enable supported Paystack checkout when production readiness is satisfied.",
            },
        )

    assert response.status_code == 200, response.text
    rails = response.json()["rails"]
    paystack = next(rail for rail in rails if rail["provider"] == "paystack")
    assert paystack["provider"] == "paystack"
    assert paystack["is_live"] is False
    assert paystack["deposits_enabled"] is False
    assert paystack["withdrawals_enabled"] is False
