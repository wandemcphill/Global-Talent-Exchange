...
def test_squad_roster_omits_source_fields_until_records_are_seeded() -> None:
...
def test_squad_sources_make_roster_authoritative_and_selection_ready() -> None:
    with _session() as session:
...
def test_injury_and_missing_contract_block_selection_readiness() -> None:
    with _session() as session:
        club_id = _seed_club(session, player_count=11)
