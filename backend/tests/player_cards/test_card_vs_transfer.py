...
def test_player_card_purchase_does_not_affect_club_transfer_or_contract(card_session: Session) -> None:
    ...
    card_ledger_entries = (
        ...
        if hasattr(LedgerSourceTag, "PLAYER_CARD_PURCHASE")
        else []
    )

    transfer_bids_count = card_session.query(TransferBid).filter_by(player_id=player.id).count()
    ...
    assert transfer_bids_count == 0
    assert isinstance(card_ledger_entries, list)
