...
def test_create_patch_publish_join_leave_flow() -> None:
    with _client() as client:
        ...
        assert published["status"] == "open_for_join"
        ...
        assert detail["status"] == "open_for_join"
        ...
        assert left["status"] == "open_for_join"


def test_join_returns_conflict_before_publish() -> None:
    with _client() as client:
        create_response = client.post(
            "/api/competitions",
            json={
