...
def test_real_player_ingestion_keeps_congo_distinct_from_drc_when_club_exists() -> None:
    engine, session_factory = _session_factory()
    try:
        with session_factory() as session:
            england = Country(
        ...
        request = RealPlayerIngestionRequest.model_validate(
            {
                ...
                "players": [
                    {
                        "source_name": "curated-feed",
                        "source_player_key": "congo-001",
                        "canonical_name": "Congo Example",
                        "nationality": "Congo",
                        "date_of_birth": "1999-01-01",
                        "primary_position": "Winger",
                        "current_real_world_club": "Fulham",
        ...
        with session_factory() as session:
            player = session.scalar(select(Player).where(Player.full_name == "Congo Example"))
            assert player is not None
            assert player.country is not None
            assert player.country.name == "Republic of the Congo"
            assert player.current_club_id == fulham.id

            profile = session.scalar(select(RealPlayerProfile).where(RealPlayerProfile.gtex_player_id == player.id))
            assert profile is not None
            assert profile.metadata_json["canonical_mapping"]["country"]["status"] == "resolved"
            assert profile.metadata_json["canonical_mapping"]["country"]["canonical_name"] == "Republic of the Congo"
