...
def _assert_converted(session, wallet, tx, recipient) -> None:
...
def test_user_hosted_gift_converts_recipient_fan_coin_to_withdrawable_gtex_coin() -> None:
    ...
    try:
        ...
        session.commit()
        _assert_converted(session, WalletService(), tx, recipient)
    finally:
        session.close()
