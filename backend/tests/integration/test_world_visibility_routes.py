...
def test_rebuild_demo_market_restores_seeded_live_visibility_routes(tmp_path: Path) -> None:
    ...
    with _demo_operator_environment(database_url=database_url, media_root=media_root):
        ...
        with patch.object(app_main, "_start_deferred_startup", autospec=True):
            ...
            app.state.container.metrics.refresh_from_database = lambda: None
            ...
            try:
                with TestClient(app) as client:
                    ...
                    rankings = client.get("/api/national-team-engine/rankings")
                    assert rankings.status_code == 200, rankings.text
                    assert rankings.json()

                    lifecycle = client.get(f"/api/national-team-engine/competitions/{competition_id}/lifecycle")
                    assert lifecycle.status_code == 200, lifecycle.text
                    assert lifecycle.json()

                    presentation = client.get(f"/api/national-team-engine/competitions/{competition_id}/presentation")
