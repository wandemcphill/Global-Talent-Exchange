...
def test_admin_scan_creates_incidents_from_repeated_gifts_and_rewards(
    client,
    demo_seed,
    ...
    promo_pool_response = client.post(
        ...
        json={
            "amount": "200.0000",
            "unit": "coin",
            "reference": "integrity-test-promo-pool",
            "note": "Fund promo pool for integrity reward settlement test.",
        },
    )
    ...
    my_score_response = client.get("/api/integrity-engine/me/score", headers=sender_headers)
    ...
    incidents_response = client.get("/api/integrity-engine/me/incidents", headers=sender_headers)
    assert incidents_response.status_code == 200, incidents_response.text
    incidents = incidents_response.json()
    assert len(incidents) >= 1

    resolve_response = client.post(
        f"/api/admin/integrity-engine/incidents/{incidents[0]['id']}/resolve",
        headers=admin_headers,
