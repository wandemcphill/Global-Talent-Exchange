...
def test_settle_marketplace_transaction_applies_fee_destination_and_overrides(session) -> None:
    ...
    assert wallet.get_balance(session, wallet.get_user_account(session, seller, LedgerUnit.CREDIT)) == Decimal('19.0000')
    assert wallet.get_balance(session, wallet.ensure_platform_burn_account(session, LedgerUnit.CREDIT)) == Decimal('1.0000')
    assert {
        entry.transaction_type
