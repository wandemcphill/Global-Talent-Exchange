...
def test_collect_match_entry_supports_coin_and_fancoin_payments(session) -> None:
    ...
    assert coin_result.fee_amount == Decimal("0.3000")
    assert coin_result.net_amount == Decimal("0.7000")
    ...
    assert wallet.get_balance(session, coin_escrow) == Decimal("0.7000")
    assert wallet.get_balance(session, coin_platform) == Decimal("-4.7000")

    assert credit_result.gross_amount == Decimal("100.0000")
    assert credit_result.fee_amount == Decimal("30.0000")
    assert credit_result.net_amount == Decimal("70.0000")
    ...
    assert wallet.get_balance(session, credit_escrow) == Decimal("70.0000")
    assert wallet.get_balance(session, credit_platform) == Decimal("-470.0000")
...
def test_settle_marketplace_transaction_applies_fee_destination_and_overrides(session) -> None:
    ...
    assert coin_result.fee_amount == Decimal("0.8000")
    assert coin_result.seller_net_amount == Decimal("3.2000")
    assert wallet.get_balance(session, wallet.get_user_account(session, buyer, LedgerUnit.COIN)) == Decimal("6.0000")
    assert wallet.get_balance(session, wallet.get_user_account(session, seller, LedgerUnit.COIN)) == Decimal("3.2000")
    assert wallet.get_balance(session, wallet.ensure_platform_account(session, LedgerUnit.COIN)) == Decimal("-10.0000")
    assert wallet.get_balance(session, wallet.ensure_trade_fee_account(session, LedgerUnit.COIN)) == Decimal("0.8000")
...
def test_governor_adjusts_pricing_conversion_and_rewards(session) -> None:
    ...
    assert settlement.net_amount == Decimal("0.5950")
