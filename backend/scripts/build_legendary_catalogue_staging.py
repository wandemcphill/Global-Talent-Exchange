...
def main() -> int:
    ...
    parser.add_argument("--max-per-country", type=int, default=35)
    ...
    if len(selected) < args.target_count:
        raise SystemExit(
            f"Only {len(selected)} diverse candidates were available; refusing to invent records for target {args.target_count}."
        )
        ...
        json.dumps(
            {
                ...
                "unique_source_candidates": len(candidates),
                "max_per_country": args.max_per_country,
            },
            indent=2,
        )
