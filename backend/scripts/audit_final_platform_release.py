    ...
    for path in frontend.rglob("*.dart"):
        ...
        for needle in needles:
            for match in re.finditer(re.escape(needle), text):
                ...
                if (
                    "baseUrl =" in line
                    and "GteBackendMode.live"
                    in text[line_start : min(len(text), line_end + 240)]
                ):
                    warnings.append(
                        f"{path.relative_to(REPO)}:{needle}:injectable_ui_default"
                    )
                else:
...
def audit() -> dict[str, object]:
    ...
    violations.extend(
        {"finding": "production_frontend_localhost", "surface": finding}
        for finding in localhost_violations
    )
    warnings.extend(
        {"finding": "injectable_ui_localhost_default", "surface": finding}
        for finding in localhost_warnings
    )

    repository_source = _read(frontend / "data" / "gte_api_repository.dart")
    if (
        "liveThenFixture" in repository_source
        and "never enables a silent fixture fallback" not in repository_source
    ):
        violations.append(
            {
                "finding": "deprecated_backend_mode_contract_missing",
    ...
    if "use_build_context_synchronously: ignore" in analysis:
        violations.append(
            {
                "finding": "global_async_context_lint_suppressed",
                "surface": "frontend/analysis_options.yaml",
            }
        )

    startup = _read(backend / "app" / "main.py") + _read(
    ...
    for gate in required_gates:
        if gate not in workflow:
            violations.append(
                {"finding": "economic_release_gate_missing", "surface": gate}
            )
