...
def audit_holdings(*, database_url: str | None) -> dict[str, Any]:
    ...
    try:
        with session_factory() as session:
            ...
            market_by_player = {
                row.player_id: row for row in market_rows
            }
            active_markets = [row for row in market_rows if row.status == "active"]

            negative_market_supply = sum(
                int((row.total_shares or 0) < 0 or (row.circulating_shares or 0) < 0)
                for row in market_rows
            )
            circulation_over_supply = sum(
                int((row.circulating_shares or 0) > (row.total_shares or 0))
                for row in market_rows
            )
            active_zero_price = sum(
                int(row.share_price_coin is None or row.share_price_coin <= 0)
                for row in active_markets
            )
            active_negative_liquidity = sum(
                int(row.liquidity_coin < 0)
                for row in active_markets
            )
