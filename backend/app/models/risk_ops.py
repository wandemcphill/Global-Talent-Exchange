...
class AmlCase(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    status: Mapped[RiskCaseStatus] = mapped_column(
        Enum(RiskCaseStatus, name="risk_case_status", native_enum=False),
        nullable=False,
        default=RiskCaseStatus.OPEN,
        server_default="open",
    )
    amount_signal: Mapped[Decimal] = mapped_column(
    ...
    assigned_admin_user_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True
    )
...
class RiskSignal(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    source: Mapped[str] = mapped_column(
        String(48), nullable=False, default="manual", server_default="manual", index=True
    )
    confidence_score: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, default=Decimal("0.00"), server_default="0.00"
    )
...
class RiskAction(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    source_rule_key: Mapped[str] = mapped_column(
        String(96), nullable=False, default="manual", server_default="manual", index=True
    )
    ...
    fraud_case_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("fraud_cases.id", ondelete="SET NULL"), nullable=True, index=True
    )
