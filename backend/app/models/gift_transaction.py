...
class GiftTransaction(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    source_ledger_unit: Mapped[LedgerUnit] = mapped_column(Enum(LedgerUnit, name="gift_source_ledger_unit", native_enum=False), nullable=False, default=LedgerUnit.CREDIT, server_default=LedgerUnit.CREDIT.value)
    destination_ledger_unit: Mapped[LedgerUnit] = mapped_column(Enum(LedgerUnit, name="gift_destination_ledger_unit", native_enum=False), nullable=False, default=LedgerUnit.COIN, server_default=LedgerUnit.COIN.value)
    economic_conversion_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("economic_conversions.id", ondelete="SET NULL"), nullable=True, unique=True, index=True)
    ...
    status: Mapped[GiftTransactionStatus] = mapped_column(Enum(GiftTransactionStatus, name="gift_transaction_status", native_enum=False), nullable=False, default=GiftTransactionStatus.SETTLED, server_default="settled")
...
class GiftStats(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    total_fan_coin_received: Mapped[Decimal] = mapped_column(Numeric(20, 4), nullable=False, default=Decimal("0.0000"), server_default="0.0000")
...
class GiftAbuseFlag(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    gift_transaction_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("gift_transactions.id", ondelete="SET NULL"), nullable=True, index=True)
