...
class PlayerImportService:
    ...
    def get_job(self, job_id: str) -> tuple[PlayerImportJob, list[PlayerImportItem]]:
        ...
        items = list(
            self.session.scalars(
                select(PlayerImportItem)
                .where(PlayerImportItem.job_id == job_id)
                .order_by(PlayerImportItem.row_number.asc())
            ).all()
        )
        return job, items

    def create_job(
        self, *, actor: User, source_type: str, source_label: str, rows: list[dict[str, object]], commit: bool
    ) -> tuple[PlayerImportJob, list[PlayerImportItem]]:
        job = PlayerImportJob(
        ...
        job.status = (
            PlayerImportJobStatus.PROCESSED
            if commit and failed == 0
            else PlayerImportJobStatus.PARTIAL if commit else PlayerImportJobStatus.DRAFT
        )
        if failed == len(rows):
    ...
    def create_card_supply_job(
        self, *, actor: User, source_label: str, rows: list[dict[str, object]], commit: bool
    ) -> tuple[PlayerImportJob, list[PlayerImportItem]]:
        job = PlayerImportJob(
            created_by_user_id=actor.id,
            source_type="card_supply",
            source_label=source_label.strip(),
            status=PlayerImportJobStatus.DRAFT,
        )
        self.session.add(job)
    ...
    def generate_youth_batch(
        self, *, actor: User, club_id: str | None, count: int, nationality_code: str, region_label: str
    ) -> tuple[PlayerImportJob, list[PlayerImportItem], list[YouthProspect]]:
        club = self._resolve_club(actor=actor, club_id=club_id)
        rows: list[dict[str, object]] = []
        prospects: list[YouthProspect] = []
        job = PlayerImportJob(
            created_by_user_id=actor.id,
            source_type="youth_generation",
            source_label=f"Youth generation for {club.club_name}",
            status=PlayerImportJobStatus.PROCESSED,
        )
        self.session.add(job)
        self.session.flush()
        items: list[PlayerImportItem] = []
        for idx in range(1, count + 1):
            payload = self._random_youth_payload(
                nationality_code=nationality_code, region_label=region_label, club_id=club.id
            )
            item = PlayerImportItem(
                job_id=job.id,
                row_number=idx,
    ...
    def list_prospects_for_club(self, club_id: str) -> list[YouthProspect]:
        return list(
            self.session.scalars(
                select(YouthProspect).where(YouthProspect.club_id == club_id).order_by(YouthProspect.created_at.desc())
            ).all()
        )
    ...
    def _upsert_player_from_payload(self, payload: dict[str, object]) -> Player:
        ...
        external_id = (
            str(payload.get("external_source_id") or payload.get("full_name")).strip().lower().replace(" ", "-")
        )
        player = self.session.scalar(
            select(Player).where(Player.source_provider == provider, Player.provider_external_id == external_id)
        )
        dob = None
        age = payload.get("age")
        if age is not None:
        ...
        if player is None:
            player = Player(
                source_provider=provider, provider_external_id=external_id, full_name=str(payload["full_name"]).strip()
            )
    ...
    def _apply_card_supply(
        self, *, actor: User, payload: dict[str, object], source_label: str
