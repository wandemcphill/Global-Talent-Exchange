...
from .seed_catalog import CATALOG_VERSION, build_seed_catalog
...
class ManagerMarketService:
    ...
    def bootstrap_market(self, app: FastAPI, session: Session) -> None:
    ...
    def _bootstrap_db(self, app: FastAPI, session: Session) -> None:
        if getattr(app.state, "manager_market_catalog_version", None) == CATALOG_VERSION:
            return
        ...
        self._sync_catalog_entries(session)
        self._ensure_default_competitions(session)
        session.flush()
    ...
    def _sync_catalog_entries(self, session: Session) -> None:
        existing_entries = {
            row.manager_id: row
            for row in session.scalars(select(ManagerCatalogEntry)).all()
        }
        active_copy_counts = {
            manager_id: int(count)
            for manager_id, count in session.execute(
                select(ManagerHolding.manager_id, func.count())
                .where(ManagerHolding.status.in_(["owned", "listed"]))
                .group_by(ManagerHolding.manager_id)
            ).all()
        }
        ...
        for raw in build_seed_catalog():
            ...
            circulating = active_copy_counts.get(manager_id, 0)
            target_total = max(int(raw["supply_total"]), circulating)
            target_available = max(0, target_total - circulating)
            entry = existing_entries.get(manager_id)
            ...
            if entry is None:
                entry = ManagerCatalogEntry(
                    ...
                    supply_total=target_total,
                    supply_available=target_available,
                )
                session.add(entry)
                existing_entries[manager_id] = entry
                continue

            entry.display_name = raw["name"]
            entry.rarity = raw["rarity"]
            entry.mentality = raw["mentality"]
            entry.tactics = raw["tactics"][:4]
            entry.traits = raw["traits"][:4]
            entry.substitution_tendency = substitution
            entry.philosophy_summary = raw["philosophy"]
            entry.club_associations = raw.get("club_associations", [])
            entry.supply_total = target_total
            entry.supply_available = target_available

    def _ensure_default_competitions(self, session: Session) -> None:
        existing_codes = set(
            session.scalars(select(ManagerCompetitionSetting.code)).all()
        )
