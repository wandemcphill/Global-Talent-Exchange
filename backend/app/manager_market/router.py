...
def get_filters(
    request: Request, session: Session = Depends(get_session), service: ManagerMarketService = Depends(get_service)
) -> ManagerFilterMetadataView:
...
def list_catalog(
    ...
    return service.list_catalog(
        request.app, session, search=search, tactic=tactic, trait=trait, mentality=mentality, rarity=rarity, limit=limit
    )
...
def get_competition_runtime(
    ...
    return service.preview_competition_runtime(
...
def create_listing(
    ...
    try:
        result = service.create_listing(
            request.app, session, current_user, payload.asset_id, payload.asking_price_credits
        )
        session.commit()
        return result
    except ManagerMarketError as exc:
...
def swap_trade(
    ...
    try:
        result = service.swap_trade(
            request.app,
            session,
            current_user,
            payload.proposer_asset_id,
            payload.requested_asset_id,
            payload.cash_adjustment_credits,
        )
        session.commit()
        return result
    except (ManagerMarketError, InsufficientBalanceError) as exc:
...
def compare_managers(
    request: Request,
    left_manager_id: str = Query(...),
    right_manager_id: str = Query(...),
    session: Session = Depends(get_session),
    service: ManagerMarketService = Depends(get_service),
) -> ManagerComparisonView:
...
def update_supply(
    manager_id: str,
    payload: ManagerSupplyUpdateRequest,
    request: Request,
    session: Session = Depends(get_session),
    actor: User = Depends(get_current_admin),
    service: ManagerMarketService = Depends(get_service),
