...
def _anchors(runtime: Any, attacks_right: bool) -> dict[str, dict[str, float]]:
    ...
    for group_index, size in enumerate(sizes):
        ...
        for local_index in range(size):
            result[outfield[cursor + local_index]] = {
                "x": float(base_x[group_index]),
                "y": float(ys[local_index]),
            }
        cursor += size
    return result
...
def _line(runtime: Any, player_id: str) -> str:
    ...
    outfield = runtime.lineup[1:]
    cursor = 0
    for index, size in enumerate(_line_sizes(runtime)):
        group = outfield[cursor : cursor + size]
        if player_id in group:
            if index == 0:
                return "defense"
            if index == len(_line_sizes(runtime)) - 1:
                return "attack"
            return "midfield"
        cursor += size
    return "midfield"
    ...
    *,
    ...
    if active_event is not None:
        ...
        if player.player_id == active_event.view.primary_player_id:
            ...
            position["x"] += (target["x"] - position["x"]) * (0.16 * event_weight)
            position["x"] += depth_offset * direction * 0.18
    ...
    position["x"] += sin((time_seconds * 0.55) + phase_x) * amplitude
    position["y"] += cos((time_seconds * 0.47) + phase_y) * amplitude * 1.25
    position["x"] = _clamp(position["x"])
    position["y"] = _clamp(position["y"])
...
def build_player_payloads(
    *,
    ...
    for runtime in (home_runtime, away_runtime):
        attacks_right = _side_attacks_right(runtime.view.side, home_attacks_right)
        anchors = _anchors(runtime, attacks_right)
...
def build_ball_payload(
    *,
    ...
    if origin is None:
        origin = dict(owner_position or {"x": 50.0, "y": 50.0})
    if target is None:
    ...
    return {
        ...
        "state": state,
        "spin": {"x": 0.0, "y": 0.35, "z": 0.8} if state == "in_flight" else None,
        "velocity": {"x": round(velocity_x, 3), "y": round(height / 0.05, 3), "z": round(velocity_y, 3)},
    }
