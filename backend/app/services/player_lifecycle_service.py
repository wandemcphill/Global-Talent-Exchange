...
class PlayerLifecycleService:
    ...
    def _sync_regen_state(
        ...
        *,
        ...
        policy_service = RegenCareerPolicyService(self.session)
        policy_context = None
        try:
            policy_context = policy_service.assess(
                player.id,
                reference_on=reference_on,
                playing_time=playing_time_ratio,
            )
        ...
        if policy_context is not None:
            ...
            state["expected_longevity_months"] = assessment.expected_longevity_months
            state["retirement_watch"] = assessment.should_enter_retirement_watch
            state["eligible_for_retirement_decision"] = assessment.eligible_for_retirement_decision
            state["policy_drivers"] = list(assessment.drivers)
            state["retirement_policy_status"] = "assessed"

            retired = already_retired or assessment.eligible_for_retirement_decision
            phase = (
                "retired"
                if retired
