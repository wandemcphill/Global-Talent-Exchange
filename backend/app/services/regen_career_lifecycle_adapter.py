    ...
    *,
    ...
    try:
        if policy_context is None:
            # Disable the legacy fixed retirement cutoff for this lifecycle pass.
            ...
            if age_months is None:
                ...
                # Let the existing, battle-tested retirement side effects run,
                # but make its clock equal the authoritative GTEX season age and
                # let the Phase 6 policy decide whether the gate is open.
                regen_date = _subtract_months(reference_on, age_months)
                regen.generated_at = datetime.combine(
                    regen_date,
                    original_generated_at.timetz(),
                )
                regen_config = replace(
                    self.settings.regen_generation,
