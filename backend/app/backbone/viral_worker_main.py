...
from app.backbone.scale_worker_runtime import ScaleTopicConsumerService, build_worker_app, viral_dispatch_handler
...
def main() -> None:
    ...
    service_name = settings.observability_service_name or f"{settings.kafka_client_id}-viral-worker"
    configure_logging(
    ...
    consumer = KafkaJsonConsumer(
        ...
        group_id=f"{settings.kafka_client_id}-viral-workers",
        client_id=f"{settings.kafka_client_id}-viral-worker",
        topics=KafkaJsonConsumer.topic_names(
            prefix=settings.kafka_topic_prefix,
            topics=("viral.clip.dispatch.requested",),
        ),
    )
    runtime = ScaleTopicConsumerService(
        consumer=consumer,
        session_factory=database.session_factory,
        handler=viral_dispatch_handler(app=app),
        consumer_name="viral-dispatch-worker",
