...
class PlayerValueSnapshotRecord(UUIDPrimaryKeyMixin, CreatedAtMixin, Base):
    ...
    snapshot_type: Mapped[str] = mapped_column(
        String(32), nullable=False, default="intraday", server_default="intraday", index=True
    )
    ...
    config_version: Mapped[str] = mapped_column(
        String(64), nullable=False, default="baseline-v1", server_default="baseline-v1"
    )
...
class PlayerValueDailyCloseRecord(UUIDPrimaryKeyMixin, CreatedAtMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("player_id", "close_date", name="uq_player_value_daily_closes_player_date"),)
...
class PlayerValueRecomputeCandidateRecord(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("player_id", name="uq_player_value_recompute_candidates_player_id"),)
    ...
    requested_tempo: Mapped[str] = mapped_column(
        String(24), nullable=False, default="hourly", server_default="hourly", index=True
    )
