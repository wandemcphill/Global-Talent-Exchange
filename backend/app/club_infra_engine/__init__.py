...
class ClubInfraService:
    ...
    def _club_for_user(self, user: User) -> ClubProfile:
        club = self.session.scalar(
            select(ClubProfile).where(ClubProfile.owner_user_id == user.id).order_by(ClubProfile.created_at.asc())
        )
        if club is None:
    ...
    def dashboard_for_club(self, *, club_id: str, viewer: User | None = None) -> dict[str, object]:
        ...
        if viewer is not None:
            holding = self.session.scalar(
                select(ClubSupporterHolding).where(
                    ClubSupporterHolding.club_id == club.id, ClubSupporterHolding.user_id == viewer.id
                )
            )
        ...
        prestige_index = int(
            stadium.level * 20
            + facilities.branding_level * 10
            + facilities.academy_level * 8
            + stadium.prestige_bonus_bps / 100
        )
        insights = [
        ...
        if token.holder_count >= 25:
            insights.append(
                "Supporter token traction is building. Story feed and gifting surfaces should feel stickier."
            )
        return {
            "club_id": club.id,
    ...
    def upgrade_stadium(self, *, actor: User, target_level: int):
        ...
        pricing = {
            ...
            for item in EconomyConfigService(self.session).list_service_pricing(active_only=False)
        }
        ...
        if price_rule is not None:
            facilities.upkeep_cost_fancoin = Decimal(facilities.upkeep_cost_fancoin) + Decimal(
                price_rule.price_fancoin_equivalent
            ) / Decimal("10")
        self.session.flush()
        return self.dashboard_for_club(club_id=club.id, viewer=actor)

    def upgrade_facility(
        self, *, actor: User, facility_key: str, increment: int = 1, current_season_number: int | None = None
    ):
        club = self._club_for_user(actor)
    ...
    def support_club(self, *, actor: User, club_id: str, quantity: int):
        ...
        holding = self.session.scalar(
            select(ClubSupporterHolding).where(
                ClubSupporterHolding.club_id == club.id, ClubSupporterHolding.user_id == actor.id
            )
        )
        created = False
        if holding is None:
            created = True
            holding = ClubSupporterHolding(
                club_id=club.id,
                user_id=actor.id,
                token_balance=0,
                influence_points=0,
                is_founding_supporter=token.holder_count < 25,
            )
