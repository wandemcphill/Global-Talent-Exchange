...
def list_story_feed(limit: int = 50, story_type: str | None = None, country_code: str | None = None, session: Session = Depends(get_session)):
    service = StoryFeedService(session)
    return [StoryFeedItemResponse.model_validate(item, from_attributes=True) for item in service.list_feed(limit=limit, story_type=story_type, country_code=country_code)]
